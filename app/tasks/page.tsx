...
import { CheckSquare, Clock, Plus, LayoutGrid, List as ListIcon, Search, User, X, CheckCircle2, Circle, ArrowRight, ArrowLeft, Loader2, Paperclip, FileText, Hand, FolderKanban, Building2, Briefcase } from 'lucide-react'
...
export default function TasksPage() {
  ...
  const claimTask = async (e: React.MouseEvent, taskId: string) => {
    ...
    if (!error) {
      toast.success('Zadanie jest Twoje!', { id: toastId })
      fetchData()
  ...
  const handleAddChecklist = async (e: React.FormEvent) => {
    ...
    if (!error) {
      setSelectedTask({ ...selectedTask, checklists: updatedChecklists, completion_percentage: percentage })
      setNewChecklistItem('')
      fetchData()
    }
  }
  ...
  const handleAddAttachment = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('tasks').update({ attachments: updatedAttachments }).eq('id', selectedTask.id)
    if (!error) {
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('tasks').insert([taskDataToInsert])
    
    if (!error) {
      setFormData({ title: '', description: '', owner_id: '', department_id: '', project_id: '', case_id: '', deadline: '', status: 'to_do', priority: 'medium' })
      setIsModalOpen(false)
      fetchData()
      toast.success('Zadanie wrzucone na tablicę!')
    } else {
  ...
  // WERYFIKACJA UPRAWNIEŃ DO TWORZENIA ZADAŃ
      ...
      >
        ...
        <div className="flex justify-between items-start mb-2 relative z-10">
          <div className="flex gap-1.5 flex-wrap">
            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${task.priority === 'high' ? 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 border-red-100 dark:border-red-800/50' : task.priority === 'medium' ? 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-500 border-yellow-100 dark:border-yellow-800/50' : 'bg-slate-50 dark:bg-slate-700 text-slate-500 dark:text-slate-400 border-slate-100 dark:border-slate-600'}`}>
              {task.priority === 'high' ? 'Pilne' : task.priority === 'medium' ? 'Średnie' : 'Niskie'}
            </span>
            
            {task.projects && (
              <span className="px-2 py-0.5 rounded text-[10px] font-bold tracking-wider border bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 border-purple-100 dark:border-purple-800/50 flex items-center gap-1">
                <FolderKanban size={10} /> {task.projects.name}
              </span>
            )}

            {task.cases && (
              <span className="px-2 py-0.5 rounded text-[10px] font-bold tracking-wider border bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border-indigo-100 dark:border-indigo-800/50 flex items-center gap-1">
                <Briefcase size={10} /> {task.cases.case_number}
              </span>
            )}
          </div>
        ...
        <h3 className="font-bold text-slate-900 dark:text-white mb-3 leading-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors relative z-10">
          {task.title}
        </h3>
        ...
        {task.checklists?.length > 0 && (
          <div className="mb-3 relative z-10">
            <div className="flex justify-between items-center text-[10px] font-bold text-slate-400 dark:text-slate-500 mb-1">
              <span>Postęp ({task.completion_percentage}%)</span>
              <span>{task.checklists.filter((c: any) => c.completed).length}/{task.checklists.length}</span>
            </div>
            <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden">
              <div className="bg-green-500 h-1.5 rounded-full transition-all duration-500" style={{ width: `${task.completion_percentage || 0}%` }}></div>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-50 dark:border-slate-700/50 relative z-10">
          ...
          {isUnassigned ? (
            task.departments ? (
              <button onClick={(e) => claimTask(e, task.id)} className={`px-2 py-1 text-[10px] font-bold uppercase tracking-widest rounded-lg shadow-sm transition-colors flex items-center gap-1 ${isForMyDept ? 'bg-orange-500 hover:bg-orange-600 text-white animate-pulse' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600'}`}>
                {isForMyDept ? <Hand size={12}/> : <Building2 size={12}/>} 
                {isForMyDept ? 'Twój Pion: Biorę to!' : `Dla: ${task.departments.name}`}
              </button>
            ) : (
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium">
              <div className="w-5 h-5 rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold text-[9px] shrink-0">
                {task.owner ? `${task.owner.first_name.charAt(0)}${task.owner.last_name.charAt(0)}` : '?'}
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {task.deadline && (
              <div className={`flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded ${isOverdue ? 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'bg-slate-50 dark:bg-slate-700 text-slate-500 dark:text-slate-400'}`}>
      ...
      <div className="flex-1 ml-64 p-4 md:p-8 transition-all flex flex-col h-screen">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 shrink-0">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-3 transition-colors">
              <CheckSquare className="text-green-500" size={32} /> Tablica Operacyjna
            </h1>
          </div>
          
          {/* UKRYWAMY PRZYCISK DLA ZWYKŁYCH CZŁONKÓW */}
          {isAdmin && (
            <button onClick={() => setIsModalOpen(true)} className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-blue-500/30 transition-all">
        ...
        <div className="bg-white dark:bg-slate-800 p-2 rounded-xl mb-6 border border-slate-200 dark:border-slate-700 shrink-0 shadow-sm flex flex-col md:flex-row gap-2 transition-colors">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Szukaj zadania..." className="w-full pl-10 pr-4 py-2 border-none focus:ring-0 outline-none text-slate-900 dark:text-white bg-transparent text-sm font-medium" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
          ...
          <div className="flex-1 flex gap-4 overflow-x-auto custom-scrollbar pb-4 items-start">
            {columns.map(col => (
              <div key={col.id} className={`flex-1 min-w-[300px] max-w-sm rounded-2xl flex flex-col max-h-full overflow-hidden border transition-colors ${col.color}`}>
                <div className={`p-4 border-b border-black/5 dark:border-white/5 shrink-0 flex justify-between items-center ${col.textColor}`}>
                  <h3 className="font-extrabold uppercase tracking-wider text-sm">{col.title}</h3>
                  <span className="bg-white/50 dark:bg-black/20 px-2 py-0.5 rounded-full text-xs font-bold">{filteredTasks.filter((t: any) => t.status === col.id).length}</span>
                </div>
                <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-3">
                  {filteredTasks.filter((t: any) => t.status === col.id).map((task: any) => <TaskCard key={task.id} task={task} />)}
                </div>
              </div>
            ))}
          </div>
          ...
          <>
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 shrink-0">
              <div className="flex justify-between items-start mb-4">
                <select 
                  className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold uppercase rounded-lg px-3 py-1.5 outline-none shadow-sm transition-colors"
              ...
              {selectedTask.description && <p className="text-sm text-slate-600 dark:text-slate-400 bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700">{selectedTask.description}</p>}
              
              <div className="flex flex-wrap gap-4 mt-4 text-xs font-bold text-slate-500 dark:text-slate-400">
                <div className="flex items-center gap-1.5">
                  <User size={14}/> 
                  {selectedTask.owner ? `${selectedTask.owner.first_name} ${selectedTask.owner.last_name}` : <span className="text-orange-500">Nieprzypisane</span>}
                </div>
                {selectedTask.deadline && <div className="flex items-center gap-1.5 text-orange-600 dark:text-orange-400"><Clock size={14}/> {selectedTask.deadline}</div>}
              </div>
                ...
                <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm softly-lifted space-y-3">
                  {selectedTask.projects && (
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-slate-500 dark:text-slate-400">Projekt parasolowy:</span>
                      <span className="text-xs font-bold bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 px-2 py-1 rounded flex items-center gap-1"><FolderKanban size={12}/> {selectedTask.projects.name}</span>
                    </div>
                  )}
                  {selectedTask.cases && (
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-slate-500 dark:text-slate-400">Powiązana Sprawa:</span>
                      <span className="text-xs font-bold bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-2 py-1 rounded flex items-center gap-1"><Briefcase size={12}/> {selectedTask.cases.case_number}</span>
                    </div>
                  )}
                  {selectedTask.departments && !selectedTask.owner_id && (
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-slate-500 dark:text-slate-400">Dedykowane dla Pionu:</span>
                      <span className="text-xs font-bold bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2 py-1 rounded flex items-center gap-1"><Building2 size={12}/> {selectedTask.departments.name}</span>
                    </div>
                  )}
                </div>
              )}
                ...
                <div className="space-y-2 mb-4">
                  {(selectedTask.checklists || []).map((item: any) => (
                    <div key={item.id} className="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-700 hover:border-blue-200 transition-colors group cursor-pointer" onClick={() => toggleChecklistItem(item.id)}>
                      <div className={`mt-0.5 shrink-0 ${item.completed ? 'text-green-500' : 'text-slate-300 dark:text-slate-600'}`}>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-slate-700">
            ...
            <form onSubmit={handleAddTask} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Tytuł zadania</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Konkretna Osoba</label>
                  <select className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-900 dark:text-white" value={formData.owner_id} onChange={(e) => setFormData({...formData, owner_id: e.target.value, department_id: ''})}>
                    <option value="">Do wzięcia! (Puste)</option>
                    {users.map((u: any) => <option key={u.id} value={u.id}>{u.first_name} {u.last_name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Lub Wyślij Do Pionu</label>
                  <select disabled={!!formData.owner_id} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-900 dark:text-white disabled:opacity-50" value={formData.department_id} onChange={(e) => setFormData({...formData, department_id: e.target.value})}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Projekt Parasolowy</label>
                  <select className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-900 dark:text-white" value={formData.project_id} onChange={(e) => setFormData({...formData, project_id: e.target.value})}>
                    <option value="">Brak</option>
                    {projects.map((p: any) => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Powiązana Sprawa</label>
                  <select className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-900 dark:text-white" value={formData.case_id} onChange={(e) => setFormData({...formData, case_id: e.target.value})}>
                    <option value="">Brak Sprawy</option>
                    {cases.map((c: any) => <option key={c.id} value={c.id}>{c.case_number} - {c.title.substring(0,20)}...</option>)}
                  </select>
                </div>
              </div>
              ...
              <button type="submit" disabled={isSubmitting} className="w-full py-4 mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg transition-all flex items-center justify-center gap-2">
                {isSubmitting ? <Loader2 size={20} className="animate-spin" /> : 'Zleć Zadanie'}
              </button>
            </form>
          </div>
        </div>
