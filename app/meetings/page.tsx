...
import { Calendar, Plus, Loader2, X, CheckCircle2, Users, FileText, Clock, ArrowRight, Lock, Printer, Edit2, Trash2 } from 'lucide-react'
...
export default function MeetingsPage() {
  ...
  const [editForm, setEditForm] = useState({ title: '', meeting_date: '', meeting_time: '', agenda: '' })
  ...
  const openEditModal = (meeting: any) => {
    ...
    setEditForm({ title: meeting.title, meeting_date: meeting.meeting_date, meeting_time: meeting.meeting_time || '', agenda: meeting.agenda || '' })
  ...
  const handleUpdateMeeting = async (e: React.FormEvent) => {
    ...
    const toastId = toast.loading('Aktualizowanie spotkania...')
    const { error } = await supabase.from('meetings').update(editForm).eq('id', editingMeeting.id)
    if (!error) {
      setIsEditModalOpen(false)
      fetchData()
      toast.success('Spotkanie zaktualizowane!', { id: toastId })
    } else toast.error('Błąd: ' + error.message, { id: toastId })
    setIsEditSubmitting(false)
  }

  const handleDeleteMeeting = async (id: string) => {
    const toastId = toast.loading('Usuwanie spotkania...')
    const { error } = await supabase.from('meetings').delete().eq('id', id)
    if (!error) {
      fetchData()
      toast.success('Spotkanie usunięte', { id: toastId })
    } else toast.error('Błąd: ' + error.message, { id: toastId })
  ...
  return (
    <>
      ...
      <div className="flex min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 relative overflow-hidden print:hidden">
                        ...
                        <div className="flex gap-3 mt-auto border-t border-slate-50 dark:border-slate-700/50 pt-4">
                          ...
                          {isToday && (
                            imCheckedIn ? (
                              ...
                              <button onClick={() => handleCheckIn(meeting.id, meeting.attendees)} disabled={isCheckingIn} className="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white shadow-lg shadow-orange-500/30 font-bold rounded-xl transition-all text-sm flex items-center gap-2 animate-pulse disabled:opacity-60 disabled:cursor-not-allowed">
                          ...
                          {isAdmin && (
                            <>
                              <button onClick={(e) => { e.stopPropagation(); openEditModal(meeting) }} className="ml-auto px-3 py-2 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-colors" title="Edytuj">
                                <Edit2 size={16}/>
                              </button>
                              <button onClick={(e) => { e.stopPropagation(); setConfirmDeleteMeetingId(meeting.id) }} className="px-3 py-2 text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors" title="Usuń">
                                <Trash2 size={16}/>
                              </button>
                            </>
                          )}
                  ...
                  {meetings.filter(m => m.meeting_date < todayStr).map(meeting => (
                    <div key={meeting.id} className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-100 dark:border-slate-700 shadow-sm flex items-center justify-between hover:border-blue-300 dark:hover:border-blue-700 transition-colors group cursor-pointer softly-lifted" onClick={() => openDetails(meeting)}>
                      ...
                      <div className="flex items-center gap-1">
                        {isAdmin && (
                          <>
                            <button onClick={(e) => { e.stopPropagation(); openEditModal(meeting) }} className="p-2 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors opacity-0 group-hover:opacity-100" title="Edytuj">
                              <Edit2 size={15}/>
                            </button>
                            <button onClick={(e) => { e.stopPropagation(); setConfirmDeleteMeetingId(meeting.id) }} className="p-2 text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors opacity-0 group-hover:opacity-100" title="Usuń">
                              <Trash2 size={15}/>
                            </button>
                          </>
                        )}
                        <ArrowRight className="text-slate-300 dark:text-slate-600 group-hover:text-blue-500 transition-colors ml-1" />
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {/* MODAL EDYCJI */}
        {isEditModalOpen && editingMeeting && isAdmin && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
              <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Edytuj posiedzenie</h2>
                <button onClick={() => setIsEditModalOpen(false)} className="text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors"><X size={24} /></button>
              </div>
              <form onSubmit={handleUpdateMeeting} className="p-6">
                <div className="mb-4">
                  ...
                  <input type="text" autoFocus className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-900 dark:text-white" value={editForm.title} onChange={(e) => setEditForm({...editForm, title: e.target.value})} required />
                </div>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Data</label>
                    <input type="date" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-900 dark:text-white [color-scheme:light] dark:[color-scheme:dark]" value={editForm.meeting_date} onChange={(e) => setEditForm({...editForm, meeting_date: e.target.value})} required />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Godzina</label>
                    <input type="time" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-900 dark:text-white [color-scheme:light] dark:[color-scheme:dark]" value={editForm.meeting_time} onChange={(e) => setEditForm({...editForm, meeting_time: e.target.value})} />
                  </div>
                </div>
                <div className="mb-6">
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Agenda</label>
                  <textarea className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none resize-none h-24 text-slate-900 dark:text-white" value={editForm.agenda} onChange={(e) => setEditForm({...editForm, agenda: e.target.value})} />
                </div>
                <button type="submit" disabled={isEditSubmitting} className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg transition-all flex items-center justify-center gap-2">
                  {isEditSubmitting ? <Loader2 className="animate-spin" size={20} /> : 'Zapisz zmiany'}
                </button>
              </form>
            </div>
          </div>
      ...
      <ConfirmDialog
        isOpen={!!confirmDeleteMeetingId}
        title="Usuń spotkanie"
        message="Czy na pewno chcesz usunąć to spotkanie? Tej operacji nie można cofnąć."
        variant="danger"
        onConfirm={() => confirmDeleteMeetingId && handleDeleteMeeting(confirmDeleteMeetingId)}
        onCancel={() => setConfirmDeleteMeetingId(null)}
      />
      </div>
